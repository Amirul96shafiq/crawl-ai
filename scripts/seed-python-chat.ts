 ...
 * Run: npx tsx scripts/seed-python-chat.ts
 */
...
async function main() {
  ...
  if (!chat) {
    console.error(
      'No archived chat named "Python Data Science" found. Create one first.',
    );
    process.exit(1);
  }
  ...
  if (existingCount > 0) {
    console.log(
      `Chat "${chat.title}" already has ${existingCount} messages. Skipping.`,
    );
    process.exit(0);
  }
